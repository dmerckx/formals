 ...
 * constructs terms through an argument ICalculus object.
 * 
 ...
 */
class Parser[Term,Type,Kind,Constraint](calc : ICalculus[Term,Type,Kind,Constraint]) extends StdTokenParsers {
    ...
    lexical.reserved ++=List("true","false","if","then","else","fixp","CNat","CBool","CList","CMaybe","CPair","succ","pred","iszero","Bool","Nat","All","let","fix","ref", "unit","Unit","Ref","in","natInd","Ref","raise","try","with", "class", "instance", "where","Eq","eq","Pi","Set","Bot")

    // KINDS
    def kind : Parser[Kind] =
      "*" ^^ (_ => calc.mkStar) |
      "(" ~> kind <~ ")" |
      (kind <~ "=>") ~ kind ^^ {case k1~k2 => calc.mkKArr(k1,k2)}
    ...
    def typ(ctx:Context): Parser[Type] =  // All X. typ, binds X in the context
        ...
        ("All" ~> ident <~ "::") ~ (kind <~ ".") >> {case tyvar~k => typ(tyvar::ctx) ^^ (t => calc.mkTKAll(t,k,tyvar))} |
        ("\\" ~> ident <~ "::") ~ (kind <~ ".") >> {case tyvar~k => typ(tyvar::ctx) ^^ (t => calc.mkTyAbs(t,k,tyvar))} |
    ...
    def tyarrow(ctx:Context): Parser[Type] =  // basetyp -> basetyp -> basetyp -> ... , right associative
        tyapp(ctx) ~ opt("->" ~> tyarrow(ctx)) ^^ { case t ~ None => t
                                                      case t1 ~ Some(t2) => calc.mkTArr(t1,t2)
                                                    }
    
    def tyapp(ctx:Context) : Parser[Type] =
        basetyp(ctx) ~ rep(tyapp(ctx)) ^^ { case t ~ Nil => t
                                            case t ~ l => (t/:l)( (x,y) => calc.mkTyApp(x,y)) 
                                          }
    ...
    def basetyp(ctx:Context): Parser[Type] =
        ...
        "Set" ^^ (_ => calc.mkSet) |
        "Bot" ^^ (_ => calc.mkTBot) |
        "CNat" ^^ (_ => calc.mkCNat(ctx.length)) |
        "CBool" ^^ (_ => calc.mkCBool(ctx.length)) |
        ident  ^^ (s=> { calc.mkTVar(ctxindex(ctx,s),ctx.length)}) |
        "Ref" ~> basetyp(ctx) ^^ (t => calc.mkTRef(t)) |
        "CList" ~> basetyp(ctx) ^^ (t => calc.mkCList(t,ctx.length)) |
        "CMaybe" ~> basetyp(ctx) ^^ (t => calc.mkCMaybe(t,ctx.length)) |
        ("CPair" ~> basetyp(ctx)) ~ basetyp(ctx) ^^ {case x~y => calc.mkCPair(x,y,ctx.length)} |
        "("~>typ(ctx)<~")"
                                 

    // TERMS                                 
    def term(ctx:Context): Parser[Term] = 
        ("\\" ~> ident <~ "::") ~ (kind <~ ".") >> {case tyvar~k => term(tyvar::ctx) ^^ (t => calc.mkTKAbs(t,k,tyvar))} |                          // \X. term, binds X in the context
    ...
    case class IncompleteParseException[T](parsedTerm: T) extends Exception;
    case class FailedParseException[T](failure:ParseResult[T]) extends Exception;
    ...
    def parseType(s : String) : Type = {
      typ(Nil)(new lexical.Scanner(s)) match {
                  case Success(r,next) if (next.atEnd) => r;
                  case Success(r,next) => throw new IncompleteParseException(r);
                  case failure => throw new FailedParseException(failure);
         }
